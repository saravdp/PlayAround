...
const utilizador = [
    ...
    new User(3, "Miguel", "sarauser@hotmail.com", "password126", "https://publicdomainvectors.org/photos/Female-Avatar-3.png", "user", 3)
]
//localStorage.clear();
        ...
        if (allUsers1[i].email == mail) {
            if (allUsers1[i].password == pass) {
                ...
                if (allUsers1[i].tipoUser == "admin") {
                    window.open('../html/Admin/admin-homepage.html')
                } else {
                    window.open('../html/profileUser.html')
                }
...
function register(user) {
    ...
    let newUsers = usersLocal.push(user);
...
function getlastId() {
    ...
    let lastId = JSON.stringify(allUsers[allUsers.length - 1]._id)
    return lastId
}
