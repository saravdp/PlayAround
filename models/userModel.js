class User {
    constructor(id, nome, email, password, foto, tipoUser, nivel) {
        this.id = id;
        this.nome = nome;
        this.email = email;
        this.password = password;
        this.foto = foto;
        this.tipoUser = tipoUser;
        this.nivel = nivel;
    }

    get nome() {
        return this._nome
    }
    get id() {
        return this._id
    }
    set nome(value) {
        //pode-se definir constrains
        this._nome = value
    }
    set id(value) {
        //pode-se definir constrains
        this._id = value
    }
    hello() {
        alert("hi! " + this.nome)
    }

}
const utilizador = [
    new User(0, "sara", "9150336@esmad.ipp.pt", "password123", "https://www.tenhomaisdiscosqueamigos.com/wp-content/uploads/2017/03/Avatar-1280x720.jpg", "admin", 1),
    new User(1, "joao", "9150337@esmad.ipp.pt", "password124", "data:image/png;base64,", "admin", 3),
    new User(2, "Joana", "9150338@esmad.ipp.pt", "password125", "data:image/png;base64,", "user", 3),
    new User(3, "Miguel", "migueluser@hotmail.com", "password126", "data:image/png;base64,", "user", 3)
]
//localStorage.clear();
// if (localStorage.getItem("User")!== null){
//     alert(localStorage.getItem("User"))

//     users = localStorage.getItem("User")
// }else {
//     localStorage.setItem("User", JSON.stringify(users))
// }

let users = [];
for (i in utilizador) {
    users.push(utilizador[i])
}


if (localStorage.getItem("User") != null && typeof localStorage.getItem("User") !== null) {

    users = localStorage.getItem("User")
    try {
        users = JSON.parse(JSON.stringify(users))

    } catch (e) {
        // You can read e for more info
        // Let's assume the error is that we already have parsed the payload
        // So just return that
        users = localStorage.getItem("User");
    }
}
else {
    localStorage.setItem("User", JSON.stringify(users));
}


// users[0].nome = "Joana"
// LOGIN  users[0].hello()
function updateObjectUser(click, newName, newEmail, newPassword, newFoto, newUserType) {
    for (i in JSON.parse(users)) {
        if (JSON.parse(users)[i]._id == click) {

            // JSON.parse(users)[i]._nome = newName
            // JSON.parse(users)[i].email = newEmail
            // JSON.parse(users)[i].password = newPassword
            // JSON.parse(users)[i].foto = newFoto
            // JSON.parse(users)[i].tipoUser = newUserType
            // JSON.parse(users)[i].nivel = JSON.parse(users)[i].nivel

            let newCom = []

            for (let j in JSON.parse(users)) {
                if (j == i) {
                    newCom.push(new User(click, newName, newEmail, newPassword, newFoto, newUserType, JSON.parse(users)[i].nivel))
                }
                else {
                    newCom.push(JSON.parse(users)[j])
                }
            }

            localStorage.setItem("User", JSON.stringify(newCom));
            location.reload()



            //JSON.parse(users)[i] = JSON.stringify(new User(click, newName, newEmail, newPassword, newFoto, newUserType,JSON.parse(users)[i].nivel))
            //alert("Atualizado" + JSON.stringify(JSON.parse(users)[i]))
        }
    }
}
//https://www.freecodecamp.org/news/how-javascript-implements-oop/
function login(mail, pass) {
    let allUsers1 = JSON.parse(users)
    for (let i in allUsers1) {

        if (allUsers1[i].email == mail) {
            if (allUsers1[i].password == pass) {
                sessionStorage.setItem("loggedUser", allUsers1[i]._id)
                if (allUsers1[i].tipoUser == "admin") {
                    location.replace('../html/Admin/admin-homepage.html')
                } else {
                    location.replace('../html/profileUser.html')
                }
alert("Bem vindo!")
            }
        }
    }
}
function register(user) {
    
    let newCom = []
    for (let i in JSON.parse(users)) {
  
      newCom.push(JSON.parse(users)[i])
    }
    //id, titulo, anfitriao, imagem, categoria, data, descricao, coordenadasLat,coordenadasLng, aderentes, agenda
    newCom.push(user)
  
    localStorage.setItem("User", JSON.stringify(newCom));
    alert("Já está registado! Efetue login" )
    location.reload()

    //localStorage.setItem()
}

function getlastId() {
    let allUsers = JSON.parse(localStorage.getItem("User"))
    let lastId = JSON.stringify(getAllUsers()[getAllUsers().length - 1]._id)
    return lastId
}
function getAllUsers() {
    return JSON.parse(users)
}
//delete user
function deleteUsers(idUser) {
    for (let i in JSON.parse(users)) {
        if (JSON.parse(users)[i]._id == idUser) {
            let newCom = []

            for (let j in JSON.parse(users)) {
                if (j != i) {
                    newCom.push(JSON.parse(users)[j])
                }
            }

            localStorage.setItem("User", JSON.stringify(newCom));
            location.reload()
        }
    }
    //location.reload()
}
function getUserById(idUser) {
    let users1 = getAllUsers()
    for (let i in users1) {
        if (users1[i].id == idUser)
            return users1
    }

}