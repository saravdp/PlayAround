...
for (let i in getAllCategories()) {
    ...
    content += `<option value="${getAllCategories()[i].idCategoria}">${getAllCategories()[i].titulo}</option>`
}
...
let image = document.getElementById('newFile')
let category = document.getElementById('categoriesList')
...
function addEvent() {
    ...
    addNewEvent(title.value, image.value, category.value, date.value, location1.value, description.value, agenda.value)
